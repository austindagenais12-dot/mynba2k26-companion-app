...
import { CareerState, DynamicEvent, PhoneContactRole, Prospect, SocialPost } from './src/model';
import {
...
import {
  addPhoneContact, discoverPhoneContact, generatePhoneCheckIn, markPhoneThreadRead,
  PHONE_CONTACT_ROLES, sendPhoneReply, unreadPhoneMessages
} from './src/phone';
import {
  gameForSchedule, pendingPlayerScheduleGames, playerScheduleGames, scheduleLocation,
  scheduleOpponent, scheduleProgress
...
function migrateCareerState(raw:any):CareerState {
  ...
  return {
    ...
    phoneContacts:Array.isArray(raw.phoneContacts)?raw.phoneContacts:base.phoneContacts,
    phoneMessages:Array.isArray(raw.phoneMessages)?raw.phoneMessages:base.phoneMessages,
...
export default function App(){
  ...
  const [tab,setTab]=useState<'Home'|'Career'|'Play'|'Calendar'|'Phone'|'More'>('Home');
  const [more,setMore]=useState<'Menu'|'Player'|'DraftClass'|'Animations'|'Social'|'Relationships'|'Sponsors'|'Life'|'League'|'History'|'Settings'>('Menu');
  ...
  const phoneUnread=unreadPhoneMessages(state);
  ...
  return <SafeAreaView style={styles.safe}>
    ...
    <View style={styles.app}>
      {tab!=='Phone'?<Header state={state} unread={unread} onNotifications={markNotifications}/>:null}
      <View style={styles.body}>
        ...
        {tab==='Phone'&&<PhoneScreen state={state} setState={setState} onOpenSocial={()=>goMore('Social')}/>}
        {tab==='More'&&<MoreRoot screen={more} setScreen={setMore} state={state} setState={setState} onEvent={setEvent} onPost={setSocialDetail}/>}
      </View>
      <BottomNav tab={tab} setTab={(t)=>{setTab(t);if(t==='More'&&more!=='Menu'){} }} unread={unread} phoneUnread={phoneUnread}/>
    </View>
...
function OnboardingScreen({state,onStart}:{state:CareerState;onStart:(profile:PlayerProfileInput)=>void}){
  return <SafeAreaView style={styles.safe}><StatusBar style="light"/><KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==='ios'?'padding':undefined}><ScrollView contentContainerStyle={styles.onboardingScroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    <View style={styles.onboardingHero}><View style={styles.onboardingLogo}><Text style={styles.onboardingLogoText}>2K</Text></View><Pill text="CAREER COMPANION V1.4"/><Text style={styles.onboardingTitle}>Create your prospect</Text><Text style={styles.onboardingSubtitle}>Set the identity the app will use across recruiting, social media, relationships, sponsors, messages, news and your entire career history.</Text></View>
...
function Home({state,activeStory,activeEvent,avgPts,onEvent,onNavigate,onMore}:{state:CareerState;activeStory:any;activeEvent:any;avgPts:number;onEvent:(e:any)=>void;onNavigate:(t:any)=>void;onMore:(m:any)=>void}){
  const p=state.player;const latestNews=state.news[0];return <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
    ...
    <Card><SectionTitle title="Quick actions"/><View style={styles.wrap}><Btn small label="Log Game" onPress={()=>onNavigate('Play')}/><Btn small label="Calendar" variant="outline" onPress={()=>onNavigate('Calendar')}/><Btn small label="Phone" variant="outline" onPress={()=>onNavigate('Phone')}/><Btn small label="Draft Class" variant="outline" onPress={()=>onMore('DraftClass')}/><Btn small label="Animations" variant="outline" onPress={()=>onMore('Animations')}/><Btn small label="Social" variant="outline" onPress={()=>onMore('Social')}/><Btn small label="League News" variant="outline" onPress={()=>onMore('League')}/><Btn small label="History" variant="outline" onPress={()=>onMore('History')}/></View></Card>
...
function Play({state,setState}:{state:CareerState;setState:(s:CareerState)=>void}){
  const eraTeams=teamsForSeason(state.settings.myNBASeasonStart);const scheduled=playerScheduleGames(state);const pending=pendingPlayerScheduleGames(state);const progress=scheduleProgress(state);
  const [selectedScheduleId,setSelectedScheduleId]=useState(pending[0]?.id||'');const [result,setResult]=useState<'W'|'L'>('W');const [importance,setImportance]=useState<(typeof importances)[number]>('Regular');
  const pendingKey=pending.map(game=>game.id).join('|');
  useEffect(()=>{if(!pending.some(game=>game.id===selectedScheduleId))setSelectedScheduleId(pending[0]?.id||'')},[pendingKey]);
  const selected=pending.find(game=>game.id===selectedScheduleId);const opponent=selected?scheduleOpponent(selected,state.player.team):'';
  ...
  if(!scheduled.length)return <ScrollView contentContainerStyle={styles.scroll}><SectionTitle title="Log 2K Game" side={<Pill text="CALENDAR REQUIRED" tone="warn"/>}/><Card style={{borderColor:WARN}}><Text style={styles.cardTitle}>Import this season's schedule first</Text><Text style={styles.bodyText}>A new game can only be logged against a matchup on your active MyNBA calendar. Open Calendar, select {state.player.team}, and import one or more schedule screenshots.</Text><Text style={styles.mutedSmall}>This keeps opponents, dates, home/away status and completed games synchronized.</Text></Card></ScrollView>;
  if(!pending.length)return <ScrollView contentContainerStyle={styles.scroll}><SectionTitle title="Log 2K Game" side={<Pill text="SEASON COMPLETE" tone="good"/>}/><Card><Text style={styles.cardTitle}>Every scheduled game is logged</Text><Text style={styles.bodyText}>{progress.logged} of {progress.total} calendar matchups have a confirmed game entry. Import an updated schedule if your MyNBA save contains additional games.</Text></Card></ScrollView>;
  const submit=()=>{if(!selected)return Alert.alert('Choose a calendar game','Select an unlogged matchup before entering the result.');const missing=(['pts','reb','ast','minutes'] as (keyof typeof v)[]).filter(key=>v[key].trim()==='');if(missing.length)return Alert.alert('Finish the box score',`Enter or scan ${missing.map(key=>humanizeScanKey(key as GameScanKey)).join(', ')} before processing the game.`);const n=(k:keyof typeof v)=>Number(v[k])||0;const next=logGame(state,{scheduleGameId:selected.id,result,importance,pts:n('pts'),reb:n('reb'),ast:n('ast'),stl:n('stl'),blk:n('blk'),tov:n('tov'),fgm:n('fgm'),fga:n('fga'),tpm:n('tpm'),tpa:n('tpa'),ftm:n('ftm'),fta:n('fta'),minutes:n('minutes'),plusMinus:n('plusMinus')});if(next===state)return Alert.alert('Calendar mismatch','That matchup is missing, belongs to another team or era, or was already logged. Refresh the Calendar tab and try again.');setState(next);setV(emptyGame());Alert.alert('Scheduled game added',`${selected.date} • ${scheduleLocation(selected,state.player.team)==='Home'?'vs':'@'} ${opponent}\n\nThe calendar, progression, phone, social media, storylines, sponsors, relationships and milestones are now synchronized.`)};
  ...
  const scanGame=async()=>{const raw=await scanner.scan('game stats');if(!raw)return;const fields=parseGameScreen(raw,{playerName:state.player.name,ownTeam:state.player.team,teamCodes:eraTeams}).filter(field=>field.key!=='opponent');if(!fields.length){Alert.alert('No box-score fields recognized','Show the stat headings and your player row in the same photo. The opponent is locked to the selected calendar matchup.');return}setReview({title:'Review game scan',subtitle:`Stats will be attached to the locked ${selected?.date} matchup against ${opponent}.`,rawText:raw,items:fields.map(field=>({id:field.key,label:humanizeScanKey(field.key),value:field.value,current:currentGameValue(field.key),confidence:field.confidence,payload:field}))})};
  const applyGameScan=(items:ScanReviewItem[])=>{const next={...v};items.forEach(item=>{const field=item.payload as ScannedGameField;if(field.key==='result'&&(field.value==='W'||field.value==='L'))setResult(field.value);else if(field.key==='importance'&&importances.includes(field.value as any))setImportance(field.value as (typeof importances)[number]);else if(field.key in next)(next as Record<string,string>)[field.key]=field.value});setV(next);setState(recordGameScreenScan(state,items.length));setReview(null);Alert.alert('Game form filled','Review any blank or unusual values, then log the selected calendar game.')};
  const dateLabel=(date:string)=>new Date(`${date}T12:00:00Z`).toLocaleDateString(undefined,{weekday:'short',month:'short',day:'numeric',year:'numeric',timeZone:'UTC'});
  return <KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==='ios'?'padding':undefined}><ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
    <SectionTitle title="Log Scheduled Game" side={<Pill text={`${progress.logged}/${progress.total} LOGGED`} tone="good"/>}/>
    <Card style={{borderColor:ACCENT}}><Row style={{justifyContent:'space-between',alignItems:'flex-start'}}><View style={{flex:1}}><Text style={styles.mutedSmall}>LOCKED CALENDAR MATCHUP</Text><Text style={styles.cardTitle}>{scheduleLocation(selected!,state.player.team)==='Home'?'vs':'@'} {teamNameForSeason(opponent,state.settings.myNBASeasonStart)}</Text><Text style={styles.bodyText}>{dateLabel(selected!.date)} • {scheduleLocation(selected!,state.player.team)}</Text></View><Pill text={selected!.canon==='2K Confirmed'?'2K SCHEDULE':'LEAGUE SCHEDULE'} tone={selected!.canon==='2K Confirmed'?'good':'muted'}/></Row><Text style={styles.label}>Choose an unlogged calendar game</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{gap:7}}>{pending.map(game=>{const opp=scheduleOpponent(game,state.player.team);const active=game.id===selectedScheduleId;return <Pressable key={game.id} onPress={()=>setSelectedScheduleId(game.id)} style={[styles.scheduleChip,active&&styles.scheduleChipActive]}><Text style={[styles.scheduleChipDate,active&&{color:'#fff'}]}>{game.date.slice(5)}</Text><Text style={[styles.scheduleChipTeam,active&&{color:'#fff'}]}>{scheduleLocation(game,state.player.team)==='Home'?'vs':'@'} {opp}</Text></Pressable>})}</ScrollView><Row style={{gap:8}}><Btn label="WIN" variant={result==='W'?'solid':'outline'} onPress={()=>setResult('W')}/><Btn label="LOSS" variant={result==='L'?'solid':'outline'} onPress={()=>setResult('L')}/></Row><Text style={styles.label}>Game importance</Text><View style={styles.chips}>{importances.map(i=><Pressable key={i} onPress={()=>setImportance(i)} style={[styles.chip,importance===i&&styles.chipActive]}><Text style={[styles.chipText,importance===i&&{color:'#fff'}]}>{i}</Text></Pressable>)}</View></Card>
    <Card><Row style={{justifyContent:'space-between',alignItems:'center'}}><View style={{flex:1}}><Text style={styles.cardTitle}>Box score</Text><Text style={styles.mutedSmall}>Tap the camera to fill stats. Date and opponent stay locked to the calendar.</Text></View><CameraButton label="Scan game stats" onPress={scanGame}/></Row><View style={styles.formGrid}><Field label="PTS" value={v.pts} onChange={x=>set('pts',x)} keyboard="number-pad"/><Field label="REB" value={v.reb} onChange={x=>set('reb',x)} keyboard="number-pad"/><Field label="AST" value={v.ast} onChange={x=>set('ast',x)} keyboard="number-pad"/><Field label="STL" value={v.stl} onChange={x=>set('stl',x)} keyboard="number-pad"/><Field label="BLK" value={v.blk} onChange={x=>set('blk',x)} keyboard="number-pad"/><Field label="TOV" value={v.tov} onChange={x=>set('tov',x)} keyboard="number-pad"/><Field label="FGM" value={v.fgm} onChange={x=>set('fgm',x)} keyboard="number-pad"/><Field label="FGA" value={v.fga} onChange={x=>set('fga',x)} keyboard="number-pad"/><Field label="3PM" value={v.tpm} onChange={x=>set('tpm',x)} keyboard="number-pad"/><Field label="3PA" value={v.tpa} onChange={x=>set('tpa',x)} keyboard="number-pad"/><Field label="FTM" value={v.ftm} onChange={x=>set('ftm',x)} keyboard="number-pad"/><Field label="FTA" value={v.fta} onChange={x=>set('fta',x)} keyboard="number-pad"/><Field label="MIN" value={v.minutes} onChange={x=>set('minutes',x)} keyboard="number-pad"/><Field label="+/-" value={v.plusMinus} onChange={x=>set('plusMinus',x)} keyboard="numbers-and-punctuation"/></View><Btn label="Log this scheduled game" onPress={submit}/></Card>
    {state.games.length>0?<Card><SectionTitle title="Recent games"/>{state.games.slice(0,7).map(g=><View key={g.id} style={styles.listRow}><Pill text={g.result} tone={g.result==='W'?'good':'warn'}/><View style={{flex:1}}><Text style={styles.listTitle}>{g.homeAway==='Away'?'@':'vs'} {g.opponent} • {g.pts}/{g.reb}/{g.ast}</Text><Text style={styles.mutedSmall}>{g.date} • {g.importance} • +{g.xp.toLocaleString()} XP{g.scheduleGameId?' • Calendar matched':' • Legacy log'}</Text></View></View>)}</Card>:null}
...
function CalendarScreen({state,setState}:{state:CareerState;setState:(s:CareerState)=>void}){
  ...
  const selectedTeamGames=eraGames.filter(game=>[game.awayTeam,game.homeTeam].includes(selectedTeam));const loggedForTeam=selectedTeamGames.filter(game=>gameForSchedule(state,game.id)).length;
  const visible=selectedTeamGames.filter(game=>game.date.startsWith(month)).sort((a,b)=>a.date.localeCompare(b.date));
  ...
  return <><ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}><SectionTitle title="Schedule & Calendar" side={<Pill text={seasonLabel(selectedSeason)} tone="good"/>}/>
    ...
    <Card><Row style={{justifyContent:'space-between',alignItems:'center'}}><View style={{flex:1}}><Text style={styles.label}>Team</Text><Text style={styles.mutedSmall}>{loggedForTeam} of {selectedTeamGames.length} calendar games logged</Text></View><Pill text={`${selectedTeamGames.length?Math.round(loggedForTeam/selectedTeamGames.length*100):0}% COMPLETE`} tone={loggedForTeam&&loggedForTeam===selectedTeamGames.length?'good':'muted'}/></Row><Progress value={selectedTeamGames.length?loggedForTeam/selectedTeamGames.length*100:0}/><View style={styles.chips}>{eraTeams.map(code=><Pressable key={code} onPress={()=>setSelectedTeam(code)} style={[styles.chip,selectedTeam===code&&styles.chipActive]}><Text style={[styles.chipText,selectedTeam===code&&{color:'#fff'}]}>{code}</Text></Pressable>)}</View><Text style={styles.label}>Month</Text>{months.length?<View style={styles.chips}>{months.map(value=><Pressable key={value} onPress={()=>setMonth(value)} style={[styles.chip,month===value&&styles.chipActive]}><Text style={[styles.chipText,month===value&&{color:'#fff'}]}>{monthLabel(value)}</Text></Pressable>)}</View>:<Text style={styles.muted}>No calendar rows yet for this era.</Text>}</Card>
    <Card><SectionTitle title={`${teamNameForSeason(selectedTeam,selectedSeason)} • ${monthLabel(month)}`} side={<Pill text={`${visible.length} games`} tone="muted"/>}/>{visible.map(game=>{const home=game.homeTeam===selectedTeam;const opponent=home?game.awayTeam:game.homeTeam;const logged=gameForSchedule(state,game.id);return <View key={game.id} style={[styles.listRow,logged&&styles.loggedCalendarRow]}><View style={[styles.calendarDay,logged&&styles.calendarDayLogged]}><Text style={styles.calendarDayText}>{game.date.slice(-2)}</Text></View><View style={{flex:1}}><Text style={styles.listTitle}>{home?'vs':'@'} {teamNameForSeason(opponent,selectedSeason)}</Text><Text style={styles.mutedSmall}>{dayLabel(game.date)} • {game.source}</Text>{logged?<Text style={styles.calendarResult}>{logged.result} • {logged.pts} PTS / {logged.reb} REB / {logged.ast} AST</Text>:null}</View><Pill text={logged?'LOGGED':game.canon==='2K Confirmed'?'2K':'NBA'} tone={logged?'good':game.canon==='2K Confirmed'?'good':'muted'}/></View>})}{!visible.length?<Text style={styles.muted}>No {selectedTeam} games are recorded for this month. Import schedule screenshots to add them.</Text>:null}</Card>
...
function PhoneScreen({state,setState,onOpenSocial}:{state:CareerState;setState:(s:CareerState)=>void;onOpenSocial:()=>void}){
  type PhoneView='Home'|'Messages'|'Thread'|'Contacts'|'Calendar';
  const [view,setView]=useState<PhoneView>('Home');const [threadId,setThreadId]=useState('');const [reply,setReply]=useState('');
  const [newName,setNewName]=useState('');const [newRole,setNewRole]=useState<PhoneContactRole>('NBA Player');const [newTeam,setNewTeam]=useState('');
  const unread=unreadPhoneMessages(state);const calendarGames=playerScheduleGames(state);const calendarProgress=scheduleProgress(state);
  const latestFor=(contactId:string)=>state.phoneMessages.find(message=>message.contactId===contactId);
  const threads=state.phoneContacts.filter(contact=>latestFor(contact.id)).sort((a,b)=>state.phoneMessages.findIndex(message=>message.contactId===a.id)-state.phoneMessages.findIndex(message=>message.contactId===b.id));
  const contact=state.phoneContacts.find(item=>item.id===threadId);const thread=state.phoneMessages.filter(message=>message.contactId===threadId).slice().reverse();
  const initials=(name:string)=>name.split(' ').map(part=>part[0]).join('').slice(0,2).toUpperCase();
  const openThread=(contactId:string)=>{setThreadId(contactId);setReply('');setView('Thread');setState(markPhoneThreadRead(state,contactId))};
  const submitReply=(body=reply)=>{if(!contact||!body.trim())return;setState(sendPhoneReply(state,contact.id,body.trim()));setReply('')};
  const addContact=()=>{if(!newName.trim())return Alert.alert('Enter a contact name','Add any player, coach, scout or other person in your MyNBA universe.');setState(addPhoneContact(state,{name:newName,role:newRole,team:newTeam}));setNewName('');setNewTeam('');Alert.alert('Contact added',`${newName.trim()} can now text your player inside the companion story.`)};
  const phoneTime=new Date().toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit'});
  const appHeader=(title:string)=><View style={styles.phoneAppHeader}><Pressable onPress={()=>setView('Home')}><Text style={styles.phoneBack}>‹ Home</Text></Pressable><Text style={styles.phoneAppTitle}>{title}</Text><View style={{width:48}}/></View>;
  return <KeyboardAvoidingView style={styles.phoneScene} behavior={Platform.OS==='ios'?'padding':undefined}><View style={styles.phoneShell}>
    <View style={styles.phoneStatus}><Text style={styles.phoneStatusText}>{phoneTime}</Text><View style={styles.phoneIsland}/><Text style={styles.phoneStatusText}>▮▮▮ 5G  ▰</Text></View>
    {view==='Home'?<View style={styles.phoneHome}>
      <View><Text style={styles.phoneHeroTime}>{phoneTime}</Text><Text style={styles.phoneHeroDate}>{new Date().toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric'})}</Text></View>
      <View style={styles.phoneWidget}><Text style={styles.phoneWidgetLabel}>CAREER CALENDAR</Text>{pendingPlayerScheduleGames(state)[0]?<><Text style={styles.phoneWidgetTitle}>{pendingPlayerScheduleGames(state)[0].date.slice(5)} • {scheduleLocation(pendingPlayerScheduleGames(state)[0],state.player.team)==='Home'?'vs':'@'} {scheduleOpponent(pendingPlayerScheduleGames(state)[0],state.player.team)}</Text><Text style={styles.phoneWidgetText}>{calendarProgress.logged} of {calendarProgress.total} games logged</Text></>:<Text style={styles.phoneWidgetText}>{calendarGames.length?'All scheduled games logged':'Import a schedule in Calendar'}</Text>}</View>
      <View style={styles.phoneApps}>
        <Pressable style={styles.phoneAppTile} onPress={()=>setView('Messages')}><View style={[styles.phoneAppIcon,{backgroundColor:'#39c96b'}]}><Text style={styles.phoneAppEmoji}>💬</Text>{unread?<View style={styles.phoneAppBadge}><Text style={styles.phoneAppBadgeText}>{Math.min(99,unread)}</Text></View>:null}</View><Text style={styles.phoneAppLabel}>Messages</Text></Pressable>
        <Pressable style={styles.phoneAppTile} onPress={()=>setView('Contacts')}><View style={[styles.phoneAppIcon,{backgroundColor:'#6874f7'}]}><Text style={styles.phoneAppEmoji}>👤</Text></View><Text style={styles.phoneAppLabel}>Contacts</Text></Pressable>
        <Pressable style={styles.phoneAppTile} onPress={()=>setView('Calendar')}><View style={[styles.phoneAppIcon,{backgroundColor:'#fff'}]}><Text style={styles.calendarIconMonth}>CAL</Text><Text style={styles.calendarIconDay}>{new Date().getDate()}</Text></View><Text style={styles.phoneAppLabel}>Calendar</Text></Pressable>
        <Pressable style={styles.phoneAppTile} onPress={onOpenSocial}><View style={[styles.phoneAppIcon,{backgroundColor:'#171a20'}]}><Text style={styles.phoneAppEmoji}>#</Text></View><Text style={styles.phoneAppLabel}>Social</Text></Pressable>
      </View>
      <View style={styles.phoneDisclaimer}><Text style={styles.phoneDisclaimerText}>FICTIONAL COMPANION SIMULATION</Text><Text style={styles.phoneDisclaimerBody}>Messages are generated for your private MyNBA story. They are not real communications from the people named.</Text></View>
      <View style={styles.phoneDock}><Pressable onPress={()=>setView('Messages')} style={[styles.phoneDockIcon,{backgroundColor:'#39c96b'}]}><Text style={{fontSize:23}}>💬</Text></Pressable><Pressable onPress={()=>setState(generatePhoneCheckIn(state))} style={[styles.phoneDockIcon,{backgroundColor:ACCENT}]}><Text style={{fontSize:23}}>☎</Text></Pressable><Pressable onPress={()=>setView('Contacts')} style={[styles.phoneDockIcon,{backgroundColor:'#6874f7'}]}><Text style={{fontSize:23}}>👥</Text></Pressable></View>
    </View>:null}
    {view==='Messages'?<View style={styles.phoneAppScreen}>{appHeader('Messages')}<ScrollView contentContainerStyle={styles.phoneList} showsVerticalScrollIndicator={false}>
      <Row style={{justifyContent:'space-between',alignItems:'center'}}><Text style={styles.phoneLargeTitle}>Messages</Text><Pressable onPress={()=>setState(generatePhoneCheckIn(state))}><Text style={styles.phoneAction}>New check-in</Text></Pressable></Row>
      {threads.map(item=>{const latest=latestFor(item.id)!;const threadUnread=state.phoneMessages.filter(message=>message.contactId===item.id&&message.direction==='Incoming'&&!message.read).length;return <Pressable key={item.id} onPress={()=>openThread(item.id)} style={styles.phoneThreadRow}><View style={styles.phoneContactAvatar}><Text style={styles.phoneContactAvatarText}>{initials(item.name)}</Text></View><View style={{flex:1}}><Row style={{justifyContent:'space-between',gap:8}}><Text numberOfLines={1} style={styles.phoneThreadName}>{item.name}{item.verified?' ✓':''}</Text><Text style={styles.phoneThreadDate}>{latest.date.slice(5)}</Text></Row><Text numberOfLines={1} style={[styles.phoneThreadPreview,Boolean(threadUnread)&&{color:'#252832',fontWeight:'800'}]}>{latest.direction==='Outgoing'?'You: ':''}{latest.body}</Text><Text style={styles.phoneThreadRole}>{item.role}{item.team?` • ${item.team}`:''}</Text></View>{threadUnread?<View style={styles.phoneUnread}><Text style={styles.phoneUnreadText}>{threadUnread}</Text></View>:null}</Pressable>})}
      {!threads.length?<Text style={styles.phoneEmpty}>No conversations yet. Add a contact or request a check-in.</Text>:null}
    </ScrollView></View>:null}
    {view==='Thread'&&contact?<View style={styles.phoneAppScreen}><View style={styles.phoneThreadHeader}><Pressable onPress={()=>setView('Messages')}><Text style={styles.phoneBack}>‹ Messages</Text></Pressable><View style={{alignItems:'center',flex:1}}><View style={[styles.phoneContactAvatar,{width:34,height:34}]}><Text style={styles.phoneContactAvatarText}>{initials(contact.name)}</Text></View><Text numberOfLines={1} style={styles.phoneThreadHeaderName}>{contact.name}{contact.verified?' ✓':''}</Text><Text style={styles.phoneThreadRole}>{contact.role}{contact.team?` • ${contact.team}`:''}</Text></View><View style={{width:68}}/></View><ScrollView style={{flex:1}} contentContainerStyle={styles.phoneMessages} showsVerticalScrollIndicator={false}>{thread.map(message=><View key={message.id} style={[styles.phoneBubble,message.direction==='Outgoing'?styles.phoneBubbleOutgoing:styles.phoneBubbleIncoming]}><Text style={[styles.phoneBubbleText,message.direction==='Outgoing'&&{color:'#fff'}]}>{message.body}</Text><Text style={[styles.phoneBubbleDate,message.direction==='Outgoing'&&{color:'#ffd8d3'}]}>{message.date}</Text></View>)}</ScrollView><View style={styles.phoneReplyArea}><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{gap:6}}>{['Appreciate you.','I’ll be ready.','Let’s work.','Call you later.'].map(text=><Pressable key={text} onPress={()=>submitReply(text)} style={styles.phoneReplyChip}><Text style={styles.phoneReplyChipText}>{text}</Text></Pressable>)}</ScrollView><Row style={{gap:8,alignItems:'center'}}><TextInput value={reply} onChangeText={setReply} onSubmitEditing={()=>submitReply()} placeholder="Text Message" placeholderTextColor="#7b808a" style={styles.phoneReplyInput}/><Pressable disabled={!reply.trim()} onPress={()=>submitReply()} style={[styles.phoneSend,!reply.trim()&&{opacity:.4}]}><Text style={styles.phoneSendText}>↑</Text></Pressable></Row></View></View>:null}
    {view==='Contacts'?<View style={styles.phoneAppScreen}>{appHeader('Contacts')}<ScrollView contentContainerStyle={styles.phoneList} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}><Text style={styles.phoneLargeTitle}>Contacts</Text><View style={styles.phoneForm}><Text style={styles.phoneFormTitle}>Add anyone in your universe</Text><TextInput value={newName} onChangeText={setNewName} placeholder="Name" placeholderTextColor="#7b808a" style={styles.phoneLightInput}/><TextInput value={newTeam} onChangeText={setNewTeam} autoCapitalize="characters" placeholder="Team code (optional)" placeholderTextColor="#7b808a" style={styles.phoneLightInput}/><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{gap:6}}>{PHONE_CONTACT_ROLES.map(role=><Pressable key={role} onPress={()=>setNewRole(role)} style={[styles.phoneRoleChip,newRole===role&&styles.phoneRoleChipActive]}><Text style={[styles.phoneRoleText,newRole===role&&{color:'#fff'}]}>{role}</Text></Pressable>)}</ScrollView><Pressable onPress={addContact} style={styles.phonePrimary}><Text style={styles.phonePrimaryText}>Add contact</Text></Pressable><Pressable onPress={()=>setState(discoverPhoneContact(state))} style={styles.phoneSecondary}><Text style={styles.phoneSecondaryText}>Discover an era-accurate NBA player</Text></Pressable></View>{state.phoneContacts.slice().sort((a,b)=>a.name.localeCompare(b.name)).map(item=><Pressable key={item.id} onPress={()=>openThread(item.id)} style={styles.phoneThreadRow}><View style={styles.phoneContactAvatar}><Text style={styles.phoneContactAvatarText}>{initials(item.name)}</Text></View><View style={{flex:1}}><Text style={styles.phoneThreadName}>{item.name}{item.verified?' ✓':''}</Text><Text style={styles.phoneThreadRole}>{item.role}{item.team?` • ${item.team}`:''}</Text></View><Text style={styles.phoneChevron}>›</Text></Pressable>)}</ScrollView></View>:null}
    {view==='Calendar'?<View style={styles.phoneAppScreen}>{appHeader('Calendar')}<ScrollView contentContainerStyle={styles.phoneList} showsVerticalScrollIndicator={false}><Text style={styles.phoneLargeTitle}>Game Calendar</Text><View style={styles.phoneCalendarSummary}><Text style={styles.phoneCalendarCount}>{calendarProgress.logged}/{calendarProgress.total}</Text><View style={{flex:1}}><Text style={styles.phoneFormTitle}>games logged</Text><Text style={styles.phoneThreadPreview}>{calendarProgress.remaining} matchup{calendarProgress.remaining===1?'':'s'} remaining</Text></View></View>{calendarGames.map(game=>{const logged=gameForSchedule(state,game.id);const opponent=scheduleOpponent(game,state.player.team);return <View key={game.id} style={styles.phoneCalendarRow}><View style={[styles.phoneCalendarDate,logged&&{backgroundColor:'#dcf8e8'}]}><Text style={styles.phoneCalendarMonth}>{new Date(`${game.date}T12:00:00Z`).toLocaleDateString(undefined,{month:'short',timeZone:'UTC'}).toUpperCase()}</Text><Text style={styles.phoneCalendarDay}>{game.date.slice(-2)}</Text></View><View style={{flex:1}}><Text style={styles.phoneThreadName}>{scheduleLocation(game,state.player.team)==='Home'?'vs':'@'} {opponent}</Text><Text style={styles.phoneThreadPreview}>{logged?`${logged.result} • ${logged.pts}/${logged.reb}/${logged.ast}`:'Not logged'}</Text></View><Text style={[styles.phoneCalendarStatus,logged&&{color:'#16834b'}]}>{logged?'✓':'○'}</Text></View>})}{!calendarGames.length?<Text style={styles.phoneEmpty}>No team schedule is loaded for this season. Use the main Calendar tab to import screenshots.</Text>:null}</ScrollView></View>:null}
...
function MoreRoot({screen,setScreen,state,setState,onEvent,onPost}:{screen:any;setScreen:(s:any)=>void;state:CareerState;setState:(s:CareerState)=>void;onEvent:(e:DynamicEvent)=>void;onPost:(p:SocialPost)=>void}){
  if(screen==='Menu')return <ScrollView contentContainerStyle={styles.scroll}><SectionTitle title="More"/><View style={styles.menuGrid}>{[
    ['Player','🏀','Attributes, badges & progression'],['DraftClass','🎓','Photo import & manual draft board'],['Animations','🎮','Optional 2K26 MyPLAYER suggestions'],['Social','#','Posts, reactions & media'],['Relationships','🤝','People, memories & rivalries'],['Sponsors','💼','Deals, agent & brand value'],['Life','🌆','Events, offseason & lifestyle'],['League','📰','Eras, rosters, news & transactions'],['History','🏆','Milestones, chapters & legacy'],['Settings','⚙️','Immersion, backups & import/export']
  ...
  if(screen==='Social')return <View style={{flex:1}}><View style={{paddingHorizontal:16,paddingTop:12}}>{back}</View><Social state={state} setState={setState} onPost={onPost}/></View>;
...
const styles=StyleSheet.create({
  ...
  calendarDay:{width:42,height:42,borderRadius:13,backgroundColor:PANEL2,borderWidth:1,borderColor:BORDER,alignItems:'center',justifyContent:'center'},calendarDayLogged:{backgroundColor:'#16362a',borderColor:'#2f855f'},calendarDayText:{color:TEXT,fontSize:16,fontWeight:'900'},loggedCalendarRow:{backgroundColor:'#101d19'},calendarResult:{color:GOOD,fontSize:11,fontWeight:'800',marginTop:3},
  scheduleChip:{minWidth:82,borderRadius:12,borderWidth:1,borderColor:BORDER,backgroundColor:PANEL2,paddingHorizontal:10,paddingVertical:8},scheduleChipActive:{backgroundColor:ACCENT,borderColor:ACCENT},scheduleChipDate:{color:MUTED,fontSize:10,fontWeight:'800'},scheduleChipTeam:{color:TEXT,fontSize:12,fontWeight:'900',marginTop:2},
  phoneScene:{flex:1,backgroundColor:'#05070a',paddingHorizontal:8,paddingTop:5,paddingBottom:3},phoneShell:{flex:1,overflow:'hidden',backgroundColor:'#10141d',borderRadius:30,borderWidth:2,borderColor:'#303641'},phoneStatus:{height:34,paddingHorizontal:16,flexDirection:'row',alignItems:'center',justifyContent:'space-between',backgroundColor:'#111720'},phoneStatusText:{color:'#f7f8fb',fontSize:10,fontWeight:'800'},phoneIsland:{position:'absolute',left:'39%',right:'39%',top:7,height:18,borderRadius:10,backgroundColor:'#000'},phoneHome:{flex:1,backgroundColor:'#1b2340',paddingHorizontal:18,paddingTop:16,paddingBottom:14,justifyContent:'space-between'},phoneHeroTime:{color:'#fff',fontSize:43,fontWeight:'300',textAlign:'center'},phoneHeroDate:{color:'#dbe2ff',fontSize:13,fontWeight:'700',textAlign:'center'},phoneWidget:{backgroundColor:'rgba(255,255,255,.13)',borderRadius:18,borderWidth:1,borderColor:'rgba(255,255,255,.18)',padding:14,gap:4},phoneWidgetLabel:{color:'#aeb9e8',fontSize:9,fontWeight:'900',letterSpacing:1},phoneWidgetTitle:{color:'#fff',fontSize:18,fontWeight:'900'},phoneWidgetText:{color:'#d9e0fa',fontSize:11},phoneApps:{flexDirection:'row',justifyContent:'space-between',gap:10},phoneAppTile:{alignItems:'center',width:'22%',gap:5},phoneAppIcon:{width:55,height:55,borderRadius:15,alignItems:'center',justifyContent:'center',position:'relative',borderWidth:1,borderColor:'rgba(255,255,255,.2)'},phoneAppEmoji:{fontSize:25,color:'#fff',fontWeight:'900'},phoneAppLabel:{color:'#fff',fontSize:10,fontWeight:'700'},phoneAppBadge:{position:'absolute',right:-6,top:-6,minWidth:20,height:20,paddingHorizontal:4,borderRadius:10,backgroundColor:'#ff3b30',alignItems:'center',justifyContent:'center',borderWidth:2,borderColor:'#1b2340'},phoneAppBadgeText:{color:'#fff',fontSize:9,fontWeight:'900'},calendarIconMonth:{color:'#e03535',fontSize:8,fontWeight:'900'},calendarIconDay:{color:'#111',fontSize:23,fontWeight:'500'},phoneDisclaimer:{backgroundColor:'rgba(5,8,14,.42)',borderRadius:14,padding:11,gap:3},phoneDisclaimerText:{color:'#f6c867',fontSize:9,fontWeight:'900',letterSpacing:.7},phoneDisclaimerBody:{color:'#d7dcef',fontSize:10,lineHeight:14},phoneDock:{alignSelf:'center',flexDirection:'row',gap:20,backgroundColor:'rgba(255,255,255,.13)',borderRadius:23,paddingHorizontal:18,paddingVertical:9},phoneDockIcon:{width:43,height:43,borderRadius:13,alignItems:'center',justifyContent:'center'},phoneHomeIndicator:{position:'absolute',bottom:5,left:'36%',right:'36%',height:4,borderRadius:2,backgroundColor:'rgba(255,255,255,.72)'},
  phoneAppScreen:{flex:1,backgroundColor:'#f6f6f8'},phoneAppHeader:{height:48,paddingHorizontal:12,flexDirection:'row',alignItems:'center',justifyContent:'space-between',borderBottomWidth:StyleSheet.hairlineWidth,borderBottomColor:'#c8c9ce',backgroundColor:'rgba(250,250,252,.98)'},phoneBack:{color:'#1677e8',fontSize:13,fontWeight:'700'},phoneAppTitle:{color:'#17181c',fontSize:13,fontWeight:'900'},phoneList:{paddingHorizontal:15,paddingTop:13,paddingBottom:28},phoneLargeTitle:{color:'#111216',fontSize:29,fontWeight:'900',marginBottom:8},phoneAction:{color:'#1677e8',fontSize:12,fontWeight:'800'},phoneThreadRow:{minHeight:66,flexDirection:'row',alignItems:'center',gap:10,paddingVertical:9,borderBottomWidth:StyleSheet.hairlineWidth,borderBottomColor:'#d7d8dc'},phoneContactAvatar:{width:46,height:46,borderRadius:23,backgroundColor:'#d8dbe2',alignItems:'center',justifyContent:'center'},phoneContactAvatarText:{color:'#4c5260',fontSize:13,fontWeight:'900'},phoneThreadName:{color:'#17181c',fontSize:14,fontWeight:'800'},phoneThreadDate:{color:'#8c9098',fontSize:9},phoneThreadPreview:{color:'#777b84',fontSize:11,lineHeight:15},phoneThreadRole:{color:'#92969e',fontSize:9,fontWeight:'700',marginTop:2},phoneUnread:{minWidth:20,height:20,borderRadius:10,paddingHorizontal:5,backgroundColor:'#1677e8',alignItems:'center',justifyContent:'center'},phoneUnreadText:{color:'#fff',fontSize:9,fontWeight:'900'},phoneEmpty:{color:'#777b84',fontSize:13,lineHeight:19,textAlign:'center',paddingVertical:30},phoneThreadHeader:{height:70,paddingHorizontal:11,flexDirection:'row',alignItems:'center',borderBottomWidth:StyleSheet.hairlineWidth,borderBottomColor:'#c8c9ce',backgroundColor:'#fafafc'},phoneThreadHeaderName:{color:'#18191d',fontSize:11,fontWeight:'900',maxWidth:180},phoneMessages:{padding:12,paddingBottom:20,gap:7},phoneBubble:{maxWidth:'82%',borderRadius:17,paddingHorizontal:12,paddingVertical:8,gap:3},phoneBubbleIncoming:{alignSelf:'flex-start',backgroundColor:'#e5e5ea',borderBottomLeftRadius:5},phoneBubbleOutgoing:{alignSelf:'flex-end',backgroundColor:'#1677e8',borderBottomRightRadius:5},phoneBubbleText:{color:'#1b1c20',fontSize:13,lineHeight:18},phoneBubbleDate:{color:'#7a7e87',fontSize:8,alignSelf:'flex-end'},phoneReplyArea:{paddingHorizontal:9,paddingTop:7,paddingBottom:14,borderTopWidth:StyleSheet.hairlineWidth,borderTopColor:'#d3d4d8',backgroundColor:'#fafafc',gap:7},phoneReplyChip:{borderRadius:999,borderWidth:1,borderColor:'#cbd4df',paddingHorizontal:10,paddingVertical:6},phoneReplyChipText:{color:'#376087',fontSize:10,fontWeight:'700'},phoneReplyInput:{flex:1,minHeight:38,borderRadius:19,borderWidth:1,borderColor:'#c8cbd1',backgroundColor:'#fff',color:'#17181c',paddingHorizontal:13,fontSize:13},phoneSend:{width:34,height:34,borderRadius:17,backgroundColor:'#1677e8',alignItems:'center',justifyContent:'center'},phoneSendText:{color:'#fff',fontSize:20,fontWeight:'900'},phoneForm:{backgroundColor:'#fff',borderRadius:16,borderWidth:1,borderColor:'#dedfe3',padding:12,gap:9,marginBottom:10},phoneFormTitle:{color:'#202126',fontSize:14,fontWeight:'900'},phoneLightInput:{minHeight:40,borderRadius:10,borderWidth:1,borderColor:'#d2d4d9',backgroundColor:'#f9f9fb',color:'#17181c',paddingHorizontal:11,fontSize:12},phoneRoleChip:{borderRadius:999,borderWidth:1,borderColor:'#c8cbd2',paddingHorizontal:9,paddingVertical:6,backgroundColor:'#fff'},phoneRoleChipActive:{backgroundColor:'#1677e8',borderColor:'#1677e8'},phoneRoleText:{color:'#555b66',fontSize:9,fontWeight:'800'},phonePrimary:{minHeight:40,borderRadius:11,backgroundColor:'#1677e8',alignItems:'center',justifyContent:'center'},phonePrimaryText:{color:'#fff',fontSize:12,fontWeight:'900'},phoneSecondary:{minHeight:40,borderRadius:11,borderWidth:1,borderColor:'#bfc8d5',alignItems:'center',justifyContent:'center'},phoneSecondaryText:{color:'#376087',fontSize:11,fontWeight:'800'},phoneChevron:{color:'#a1a4ab',fontSize:25},phoneCalendarSummary:{flexDirection:'row',alignItems:'center',gap:12,backgroundColor:'#fff',borderRadius:16,padding:13,borderWidth:1,borderColor:'#dedfe3',marginBottom:10},phoneCalendarCount:{color:'#1677e8',fontSize:31,fontWeight:'900'},phoneCalendarRow:{minHeight:64,flexDirection:'row',alignItems:'center',gap:10,borderBottomWidth:StyleSheet.hairlineWidth,borderBottomColor:'#d7d8dc',paddingVertical:7},phoneCalendarDate:{width:42,height:48,borderRadius:9,backgroundColor:'#fff',borderWidth:1,borderColor:'#d5d6da',alignItems:'center',justifyContent:'center'},phoneCalendarMonth:{color:'#e13b3b',fontSize:8,fontWeight:'900'},phoneCalendarDay:{color:'#1e2025',fontSize:18,fontWeight:'700'},phoneCalendarStatus:{color:'#a9acb3',fontSize:20,fontWeight:'900'},
